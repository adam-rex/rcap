-- Multi-sector filter (OR) on contacts.sector via p_sectors text[].
-- Replaces 6-arg workspace_contacts_page with 7-arg (p_sectors defaults to empty = no filter).

drop function if exists public.workspace_contacts_page(text, int, int, text, text, text);
...
comment on function public.workspace_contacts_page (text, int, int, text, text, text, text[]) is
  'Paged contact list with optional role, organisation type, contact type, sector (ANY) filters, and search.';

grant execute on function public.workspace_contacts_page (text, int, int, text, text, text, text[]) to authenticated;
grant execute on function public.workspace_contacts_page (text, int, int, text, text, text, text[]) to service_role;
